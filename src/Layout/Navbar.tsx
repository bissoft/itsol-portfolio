...
const Navbar = () => {
  ...
  const [dynamicMenus, setDynamicMenus] = useState<MenuItem[]>(Menus);
  const router = useRouter();
  const pathname = usePathname();

  // Fetch dynamic navbar data from API
  useEffect(() => {
    fetch("/api/admin/navbar")
      .then((res) => res.json())
      .then((data) => {
        if (data?.menus && Array.isArray(data.menus) && data.menus.length > 0) {
          const converted = convertNavbarDataToMenuItems(data.menus);
          setDynamicMenus(converted);
        }
      })
      .catch(() => {
        // Fallback to static menus on error
  ...
  // 👇 Check if current route is /case-study/2 or /portfolio or /podcasts
  const isSolidNavbar =
  ...
  return (
    <div className="relative z-50">
      <header
        className={`fixed top-0 left-0 right-0 flex items-center justify-center transition-all duration-300 ${
          isScrolled || isSolidNavbar
            ? "h-16 bg-white/95 backdrop-blur-md shadow-sm border-b border-gray-100"
            : "h-20 bg-transparent"
        }`}
      >
        <nav className="px-6 flex items-center justify-between w-full max-w-7xl mx-auto h-full">
          ...
          >
            ...
            >
              <motion.div
                className={`relative flex items-center justify-center overflow-hidden transition-all duration-300 ${isScrolled || isSolidNavbar ? "w-16 h-16" : "w-20 h-20"}`}
          ...
          >
            ...
            >
              ...
              <div className="text-center mb-6">
                ...
                <h3 className="text-2xl font-bold text-gray-800">Let's Work</h3>
